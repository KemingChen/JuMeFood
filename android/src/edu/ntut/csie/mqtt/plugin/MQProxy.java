...
import com.JuMeFood.MQTTResources;
...
public class MQProxy {
	...
	private static final String MQTT_URL = "tcp://140.124.183.138:1883";
	...
	private MQProxy() {
		this.aConnectionManager = MQTTResources.getConnectivityManager();
	}
	...
	public synchronized boolean connect(String clientId, String topic,
		...
		//clientId = "hans/1s6d5f4sd3f5";
		//topic = "hans/123456";
		try {
			if (!MQTTResources.isService()) {
				SharedPreferences sp = MQTTResources.getContext()
			...
			if (isNetworkAvailable()) {
				...
				LogUtil.d(TAG, "MQtt Connected to server:", MQTT_URL," with client id:", clientId);
		...
		if (!MQTTResources.isService())
			MQTTResources.getMQTT().sendMqttConnectionStatus(
