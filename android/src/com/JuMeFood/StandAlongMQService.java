...
import edu.ntut.csie.mqtt.plugin.MQProxy;
import edu.ntut.csie.mqtt.plugin.MsgArrivedCallback;
...
public class StandAlongMQService extends Service {
	...
	try {
	    SharedPreferences sp = getSharedPreferences("PersistenceSharedPreferences",
	...
	PendingIntent pi = PendingIntent.getActivity(MQTTResources
	        .getContext().getApplicationContext(), 0, i,
	        PendingIntent.FLAG_UPDATE_CURRENT);

	LogUtil.d(TAG, "Parser mqtt message to Notification! ");
	n.setLatestEventInfo(MQTTResources.getContext(), NOTIF_TITLE,
	    msg, pi);

	// 只保留最新的 Notification 訊息
	MQTTResources.getNotificationManager().notify(0, n);
    }
	...
	Intent i = new Intent(MQTTResources.getContext()
	        .getApplicationContext(), JuMeFood.class);
