...
import com.JuMeFood.MQService.MQBinder;
import com.worklight.androidgap.WLDroidGap;

import edu.ntut.csie.mqtt.plugin.LogUtil;

public class JuMeFood extends WLDroidGap {
...
/*			task.post(new Runnable() {
			...
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					JuMeFood.this.bindService(new Intent(JuMeFood.this,
		...
		task.post(new Runnable() {
			...
			public void run() {
				...
				try {
					SharedPreferences sp = JuMeFood.this
							.getSharedPreferences(
									"PersistenceSharedPreferences",
									Context.MODE_PRIVATE);
					String message = sp.getString("message", "");
					if (StringUtils.isNotBlank(message)) {
						JuMeFood.this.callMessageHandlerRouters(message);
					}
					SharedPreferences.Editor spEditor = sp.edit();
	...
	public void onPause() {
		...
		if (mqsConnected) {
			task.post(new Runnable() {
				...
				@Override
				public void run() {
					JuMeFood.this.unbindService(conn);
				}
			});
		...
		/*task.post(new Runnable() {
			...
			@Override
			public void run() {
				JuMeFood.this.startService(new Intent(JuMeFood.this,
	...
	private void loadResource() {
		...
		// @Override
		// public void run() {
		MQTTResources.init(JuMeFood.this);
		// }
		// });
	}
	...
	private class MQServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			JuMeFood.this.mqs = ((MQBinder) service).getService();
			if (JuMeFood.this.clientId != null
					&& JuMeFood.this.topic != null) {
				JuMeFood.this.task.post(JuMeFood.this.connectMQ);
			}
			JuMeFood.this.mqsConnected = true;
		}

		@Override
		public void onServiceDisconnected(ComponentName name) {
			JuMeFood.this.mqsConnected = false;
		}
	...
	private class MQConnectTask implements Runnable {
		...
		@Override
		public void run() {
			SharedPreferences sp = JuMeFood.this.getSharedPreferences(
					"PersistenceSharedPreferences", Context.MODE_PRIVATE);
			SharedPreferences.Editor spEditor = sp.edit();
			spEditor.putString("clientId", clientId).commit();
			spEditor.putString("topic", topic).commit();
			if (JuMeFood.this.mqs.connect(clientId, topic)) {
				task.post(new Runnable() {
					@Override
					public void run() {
						NotificationManager mNotifMan = MQTTResources
	...
	public void publishMessage(final String topic, final String message,
		...
		task.post(new Runnable() {
			...
			@Override
			public void run() {
				JuMeFood.this.mqs.publishMessage(topic, message, qos);
			}
		});
	}
